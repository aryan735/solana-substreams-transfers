...
// @@protoc_insertion_point(attribute:transfers)
pub mod transfers {
    include!("transfers.rs");
    // @@protoc_insertion_point(transfers)
}
// src/pb/mod.rs
pub mod transfers {
    include!(concat!(env!("OUT_DIR"), "/transfers.rs"));
}
mkdir -p /mnt/d/solana-transfers/src/pb
cat > /mnt/d/solana-transfers/src/pb/mod.rs << 'EOF'
pub mod transfers {
    include!(concat!(env!("OUT_DIR"), "/transfers.rs"));
}
EOF